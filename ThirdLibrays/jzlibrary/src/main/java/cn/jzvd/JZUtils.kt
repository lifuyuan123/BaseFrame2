...
object JZUtils {
    ...
    fun saveProgress(context: Context, url: Any?, progress: Long) {
        ...
        if (!Jzvd.Companion.SAVE_PROGRESS) return
    ...
    fun getSavedProgress(context: Context, url: Any?): Long {
        if (!Jzvd.Companion.SAVE_PROGRESS) return 0
        val spn = context.getSharedPreferences("JZVD_PROGRESS",
                Context.MODE_PRIVATE)
    ...
    fun showStatusBar(context: Context?) {
        if (Jzvd.Companion.TOOL_BAR_EXIST) {
    ...
    fun getNavigationBarHeight(context: Context): Int {
        ...
        var var2: Int
        return if (context.resources.getIdentifier("navigation_bar_height", "dimen", "android").also { var2 = it } > 0 && !var1) context.resources.getDimensionPixelSize(var2) else 0
    }
